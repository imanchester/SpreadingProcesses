...
#basic dimensions
numvar=n+nR+1+n; #+1 for max objective 
...
asubj=append!(asubj,n+2:numvar)
aval=repeat([1,-1],outer=n)
aval=append!(aval,ones(nR+n))    #values +delta
ajval=[1, 1, 1, -1]
ajvalN=[1, 1, -1]
aj2val=ones(2*n) #check this
aj3val=ones(2*n) #check this
...
a2i=repeat(numLinCon+1:numLinCon+n,inner=2)
a2c=repeat(2+n+nR:numvar,inner=2)
a2c[1:2:2*n]=Array(numvar+3:3:dstop)
...
maketask() do task
    ...
    putvarboundslice(task,n+2,numvar-n+1,[MSK_BK_RA for i in 1:nR], [0 for i in 1:nR], [log(VV[i]/betaL) for i in 1:nR]) #add constraints on rij
    putvarboundsliceconst(task,numvar-n+1,numvar+1,MSK_BK_RA, 0, log((1-delta)/(1-deltaH))) #add constraints on vij
      

    #set up linear constraints
   	putaijlist(task,asubi, asubj, aval) ############## this one has to include delta
